  ...
  <body>
    <div id="bar">
      <ul id="nav">
        ...
        <p class="follow"><?php echo count($followers); ?> followers&nbsp;&nbsp;&nbsp;<a href="following.php?username=<?php echo $username;?>"> <?php echo count($following); ?> following</a></p></div></li>
        <li class="connection">
    ...
    <div id = "content">
      <div class="border">
        ...
        for ($i=0; $i <=count($following); $i++) {
          $followings = DB::query('SELECT user_id FROM followers WHERE user_id =:userid', array(':userid'=>$userid))[$i]['user_id'];
          $peopleIfollow = DB::query('SELECT follower_id FROM followers WHERE user_id =:userid', array(':userid'=>$followings))[$i]['follower_id'];
